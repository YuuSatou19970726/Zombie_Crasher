...
public class GameplayController : MonoBehaviour
{
    ...
    [SerializeField]
    private float min_ObstacleDelay = 10f, max_ObstacleDelay = 40f;
    ...
    void CreateObstacles(float zPos) {
        ...
        if (0 <= r && r < 7)
        {
            ...
            if (obstacleLane == 0) {
                zombieLance = Random.Range (0, 2) == 1 ? 1 : 2;
            } else if (obstacleLane == 1) {
                zombieLance = Random.Range (0, 2) == 1 ? 0 : 2;
            } else if (obstacleLane == 2) {
                zombieLance = Random.Range (0, 2) == 1 ? 1 : 0;
            }

            AddZombies (new Vector3(lanes[zombieLance].transform.position.x, 0.15f, zPos));
        }
    }
